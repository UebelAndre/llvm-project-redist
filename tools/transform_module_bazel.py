...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("output", type=Path, help="Path to write transformed MODULE.bazel")
...
def transform(source: str, version: str) -> str:
    ...
    if not removed_llvm_raw_repo_entry:
        print(
            "WARNING: did not find 'llvm-raw' in use_repo(); upstream MODULE.bazel format may have changed",
            file=sys.stderr,
        )
