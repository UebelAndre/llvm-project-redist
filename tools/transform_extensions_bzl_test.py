...
class TransformExtensionsBzlTest(unittest.TestCase):
    ...
    def test_new_local_repository_removed(self) -> None:
        self.assertNotIn("new_local_repository(", self.result)

    def test_llvm_raw_name_removed(self) -> None:
        self.assertNotIn('"llvm-raw"', self.result)

    # ---- local.bzl import removed ----

    def test_local_bzl_import_removed(self) -> None:
        self.assertNotIn("local.bzl", self.result)

    def test_http_archive_import_preserved(self) -> None:
        self.assertIn(
            'load("@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")',
            self.result,
        )

    def test_vulkan_sdk_import_preserved(self) -> None:
        self.assertIn(
            'load(":vulkan_sdk.bzl", "vulkan_sdk_setup")',
            self.result,
        )
    ...
    def test_at_llvm_raw_labels_removed(self) -> None:
        self.assertNotIn("@llvm-raw//", self.result)

    def test_labels_rewritten_to_label_call(self) -> None:
        self.assertIn(
    ...
    def test_vulkan_sdk_setup_preserved(self) -> None:
        self.assertIn('vulkan_sdk_setup(name = "vulkan_sdk")', self.result)

    # ---- module_extension preserved ----

    def test_module_extension_preserved(self) -> None:
